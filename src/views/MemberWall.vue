    ...
    <div class="max-w-4xl mx-auto">
      ...
      <template v-else>
        <div v-if="members.length > 0" class="space-y-5">
          <section v-for="group in members" :key="group.year"
            ...
            <div class="flex flex-wrap items-center gap-3 mb-6">
              ...
              <span
                class="text-xs text-gray-400 px-2 py-0.5 bg-gray-50 dark:bg-gray-800 rounded-full border border-gray-100 dark:border-gray-700">
                {{ group.members.length }} 人
              </span>
            </div>

            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2.5">
              <div v-for="member in group.members" :key="isString(member) ? member : member.name"
                ...
                <template v-else>
                  <div class="flex justify-between items-center gap-1 mb-1">
                    ...
                    <span
                      class="flex-shrink-0 text-[10px] font-medium text-[#40B3FF] bg-blue-50 dark:bg-blue-900/30 px-1.5 py-0.5 rounded-full">
                      志愿者
                    </span>
                  </div>
